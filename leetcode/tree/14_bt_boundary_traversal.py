"""Binary tree boundary traversal clockwise and anti clockwise"""
...
class Solution:
    ...
    def __left_boundary(self, root: Optional[Node], result: List[int]) -> None:
        # Helper function to traverse and collect the left boundary of the tree.
    ...
    def __right_boundary(self, root: Optional[Node], result: List[int]) -> None:
        # Helper function to traverse and collect the right boundary of the tree.
        # It starts from the root's right child and iteratively moves down to the rightmost non-leaf node.
        curr: Node = root.right
        tmp: List[int] = (
            []
        )  # Temporary storage for right boundary nodes (excluding leaves).
        while curr:
            if not self.__is_leaf(node=curr):
        ...
        # Add the collected right boundary nodes to the result list in reverse order.
        while tmp:
    ...
    def boundary_anti_clock(self, root: Optional[Node]) -> List[int]:
        # Main function to compute the boundary of the tree in an anti-clockwise manner.
        result: List[int] = []
        if root is None:
            return result
        ...
        # Collect the left boundary, leaves, and right boundary in an anti-clockwise manner.
        self.__left_boundary(root=root, result=result)
        self.__add_leaves(node=root, result=result)
        self.__right_boundary(root=root, result=result)
...
if __name__ == "__main__":
    ...
    # Compute and print the boundary traversal of the binary tree in both clockwise and anti-clockwise directions.
    print(solution.boundary_anti_clock(root=root))
