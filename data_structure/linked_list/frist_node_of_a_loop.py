"""Detect starting point of Loop in singly linked list."""
...
def find_starting_node(slow: Node, fast: Node) -> Node:
    while slow != fast:
        slow = slow.next
        fast = fast.next
    return slow
...
@time_it
def detect_cycle_optimal(head: Node) -> Node:
    if head is None:
        return None
    fast: Node = head
    slow: Node = head
    while fast and fast.next:
        ...
        fast = fast.next.next
        if slow == fast:
            return find_starting_node(slow=head, fast=fast)
