...
def find_unique_paths(i: int, j: int, dp: list[list[int]], arr: list[list[int]]) -> int:
    """
    ...
    if dp[i][j] != -1:
        return dp[i][j]
    up: int = find_unique_paths(i=i - 1, j=j, dp=dp, arr=arr)
    left: int = find_unique_paths(i=i, j=j - 1, dp=dp, arr=arr)

    dp[i][j] = up + left
    return dp[i][j]


def find_path_tabulation(n: int, m: int, arr: list[list[int]]) -> int:
    """
...
def find_path_optimal(n: int, m: int, arr: list[list[int]]) -> int:
    """
...
if __name__ == "__main__":
    ...
    dp: list[list[int]] = [[-1 for _ in range(m)] for row in range(n)]
    print(dp)
    paths: int = find_unique_paths(i=n - 1, j=m - 1, dp=dp, arr=arr)
    print(paths)
    print(find_path_tabulation(n=n, m=m, arr=arr))
    print(find_path_optimal(n=n, m=m, arr=arr))
